...
- Админ: кнопки в боте + веб-панель `/admin` (услуги, настройки, записи — без кода)
...
| Что менять | Где |
|---|---|
| Услуги и цены | Админка → **Услуги** |
| Часы, выходные, предоплата, название | Админка → **Настройки** |
| Барберы, отзывы, галерея | Соответствующие разделы админки |
| Стили Mini App | `mini-app/style.css` (для разработчика) |
| Токен бота, URL сервера | Render → Environment |
